...
(function(nr, doc, g, q, cls, skryptSRC, disp, sA, txtCnt) {
	let wszysTagi = [];
	let posTagi = [];
	...
	styl.innerHTML = `#${d} div.pbpLabel{margin:3px 8px;${disp}:${display !== 'list' ? 'inline-flex' : 'flex'}${display === 'list' ? ';justify-content:'+textAlign : ''};align-items:center;${combining ? 'border:' + borderWidth + 'px solid ' + borderColor + ';border-radius:'+borderRadius+'px;background:' + background + ';padding:2px;' : ''}}
...
#${d} .pbpTC_searchButton{${disp}:inline-flex;align-items:center;position:relative;font-size:${textSize}px;padding:3px 7px;background:#949494;border-width:3px;border-style:outset;border-color:#9e9e9e;border-radius:5px;cursor:pointer;}
...
#${d} .pbpTC_searchButton .searchInfo:after {content:"";${disp}:inline-block;position:absolute;left:calc(50% - 6px);top:100%;border-width:6px;border-style:solid;border-color:transparent;border-top:6px solid black;}
...
})(pbpTagCloud, document, 'getAttribute', 'querySelectorAll', 'class', 'script[src="https://probloggerplugins.github.io/widgets/tag-cloud.js"]', 'display', 'setAttribute', 'textContent');
